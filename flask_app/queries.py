...
"""SELECT date, group_concat(hp.player_name, '-'), home_team, home_score, group_concat(ap.player_name, '-'), away_team, away_score"""

# TODO this query isn't correct -- the multiple inner joins cause duplication of player names
game_history_query = """SELECT date, group_concat(hp.player_name, '-'), home_team, home_score, group_concat(ap.player_name, '-'), away_team, away_score FROM game INNER JOIN away_players ap ON ap.game_id = id INNER JOIN home_players hp ON hp.game_id = id GROUP BY id ORDER BY date DESC;"""